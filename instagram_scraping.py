...
def parse_data(username, user_data):
    ...
    if len(user_data['edge_owner_to_timeline_media']['edges']) > 0:
        ...
        if latest_node['node']['is_video']:
            post_type.append('Video')
        else:
            post_type.append('Photo')
        
        # Calculate the total number of likes, comments and interactions
        total_likes += latest_node['node']['edge_liked_by']['count']
        total_comments += latest_node['node']['edge_media_to_comment']['count']
        total_interactions = total_likes + total_comments

        # Check the Like and view counts disabled
        if 'hide_like_and_view_counts' in latest_node['node']:
            hide_like_and_view_counts = latest_node['node']['hide_like_and_view_counts']
        #hide_like_and_view_counts = latest_node['node'].get('hide_like_and_view_counts', False)
...
def main():
    ...
    with open('instagram.csv', 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['UserName', 'Name', 'Category', 'Followers', 'Following', 'Number of Post', 'Total Likes', 'Total Comments', 'Total Interactions', 'Posts', 'Post URLs', 'Type', 'Link', 'Like and view counts disabled']
